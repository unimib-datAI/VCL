...
def show_user_info():
    ...
    users = deepcopy(st.session_state.assistant.get_storage().get_all_users())
    
    col1, col2, col3 = st.columns([1, 1, 1])
    with col1:
        if st.button("Ricarica Utenti", use_container_width=True):
            st.rerun()
    with col2:
        if st.button("Aggiungi Nuovo Utente", use_container_width=True):
            st.session_state.admin_action = "register_info"
            st.rerun()
    with col3:
        if st.button("Rimuovi Utente", use_container_width=True):
...
def show_delete_user():
    ...
    users = st.session_state.assistant.get_storage().get_all_users()
    ...
    if st.button("Rimuovi Utente", use_container_width=True):
        if selected_user:
            if selected_user == st.session_state.username:
                ...
                result = st.session_state.authenticator.delete_user(selected_user)
                if result:
...
def show_admin():
    ...
    # Controlli iniziali
    required_keys = ["assistant", "username", "role"]
